        ...
        Alpine.data("vgProductVariants", function () {
            return {
                ...
                isButton: function (attr) {
                    return attr.display_type === "button";
                },
                ...
                findVariant: function () {
                    var selectedIds = Object.values(this.selected).map(Number).sort(function (a, b) { return a - b; });
                    if (!selectedIds.length) return null;
                    return this.variants.find(function (variant) {
                        var ids = (variant.attributes || []).map(function (a) { return a.id; }).sort(function (a, b) { return a - b; });
                        if (ids.length !== selectedIds.length) return false;
                        for (var i = 0; i < ids.length; i++) {
                            if (ids[i] !== selectedIds[i]) return false;
                        }
                        return true;
                    }) || null;
                },
